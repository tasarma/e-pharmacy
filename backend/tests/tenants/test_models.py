...
class TestTenantModel:
    ...
    def test_create_valid_tenant(self):
        ...
        with tenant_context_disabled():
            tenant = Tenant.objects.create(
                name="Valid Pharmacy", subdomain="validpharm"
            )
    ...
    def test_subdomain_validation_invalid_chars(self):
        ...
        with tenant_context_disabled():
            tenant = Tenant(name="Test", subdomain="test_invalid!")
    ...
    def test_subdomain_reserved_keywords(self):
        ...
        with tenant_context_disabled():
            tenant = Tenant(name="Admin Store", subdomain="admin")
    ...
    def test_subdomain_uniqueness(self, tenant):
        ...
        with tenant_context_disabled():
            duplicate = Tenant(name="Duplicate", subdomain=tenant.subdomain)
    ...
    def test_create_settings(self, tenant):
        ...
        with set_tenant_context(tenant=tenant):
            settings = TenantSettings.objects.create(
                tenant=tenant, store_name="My Store", email="store@example.com"
            )
    ...
    def test_settings_one_to_one(self, tenant):
        ...
        with set_tenant_context(tenant=tenant):
            TenantSettings.objects.create(
                tenant=tenant, store_name="Store 1", email="store1@example.com"
            )

            # Attempt to create duplicate
            with pytest.raises(Exception):
                TenantSettings.objects.create(
                    tenant=tenant, store_name="Store 2", email="store2@example.com"
                )
